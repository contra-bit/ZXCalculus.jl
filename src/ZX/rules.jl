...
function Base.match(::Rule{:lc}, zxg::ZXGraph{T,P}) where {T,P}
    ...
    sort!(vB)
    for v in vs
        if spider_type(zxg, v) == SpiderType.Z && (phase(zxg, v) in (1 // 2, 3 // 2))
           ...
           v1 ∉ v_matched
            for v2 in neighbors(zxg, v1)
                if spider_type(zxg, v2) == SpiderType.Z &&
                   length(searchsorted(vB, v2)) == 0 &&
                   (phase(zxg, v2) in (0, 1)) &&
                   length(searchsorted(gadgets, v2)) == 0 &&
                   v2 ∉ v_matched
                    push!(matches, Match{T}([v1, v2]))
                    push!(v_matched, v1, v2)
                end
...
function Base.match(::Rule{:gf}, zxg::ZXGraph{T,P}) where {T,P}
    ...
    gads = [
        (v, neighbors(zxg, v)[1], setdiff(neighbors(zxg, neighbors(zxg, v)[1]), [v])) for
        v in gad_ids
    ]

    for i = 1:length(gads)
        v1, v2, gad_v = gads[i]
        for j = (i+1):length(gads)
            u1, u2, gad_u = gads[j]
            if gad_u == gad_v &&
               phase(zxg, v2) in (zero(P), one(P)) &&
               phase(zxg, u2) in (zero(P), one(P))
...
function check_rule(r::Rule{:i2}, zxd::ZXDiagram{T,P}, vs::Vector{T}) where {T,P}
    ...
    if spider_type(zxd, v1) == SpiderType.H &&
       spider_type(zxd, v2) == SpiderType.H &&
       has_edge(zxd.mg, v1, v2)
...
function check_rule(r::Rule{:c}, zxd::ZXDiagram{T,P}, vs::Vector{T}) where {T,P}
    ...
    if spider_type(zxd, v1) == SpiderType.X &&
       phase(zxd, v1) == zero(phase(zxd, v1)) &&
       (degree(zxd, v1)) == 1
        if v2 in neighbors(zxd, v1)
            if spider_type(zxd, v2) == SpiderType.Z
                return true
...
function rewrite!(::Rule{:id}, zxg::ZXGraph{T,P}, vs::Vector{T}) where {T,P}
    ...
    set_phase!(
        zxg.master,
        id3,
        (mul3 * phase(zxg.master, id3) + mul1 * phase(zxg.master, id1)) * mul3,
    )
...
function check_rule(::Rule{:gf}, zxg::ZXGraph{T,P}, vs::Vector{T}) where {T,P}
    @inbounds if all(has_vertex(zxg.mg, v) for v in vs)
        ...
        if spider_type(zxg, v1) == SpiderType.Z &&
           (degree(zxg, v1)) == 1 &&
           spider_type(zxg, u1) == SpiderType.Z &&
           (degree(zxg, u1)) == 1
...
function rewrite!(::Rule{:gf}, zxg::ZXGraph{T,P}, vs::Vector{T}) where {T,P}
    ...
    set_phase!(
        zxg.master,
        idv,
        (mulv * phase(zxg.master, idv) + mulu * phase(zxg.master, idu)) * mulv,
    )
...
function check_rule(
    ::Rule{:scalar},
    zxg::Union{ZXGraph{T,P},ZXDiagram{T,P}},
    vs::Vector{T},
) where {T,P}
    @inbounds v = vs[1]
    if has_vertex(zxg.mg, v)
