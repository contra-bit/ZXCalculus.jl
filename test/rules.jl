...
@testset "id rule" begin
  zxd_id = ZXDiagram(1)
  push_gate!(zxd_id, Val{:Z}(), 1)
  zxg_id = ZXGraph(zxd_id)
  replace!(Rule{:id}(), zxg_id)
  @test contains_only_bare_wires(zxg_id) == true
end
